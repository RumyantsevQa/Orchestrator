...
![QA Knowledge OS overview](docs/assets/overview.svg)

## Навигация

| Раздел | Зачем открыть |
| --- | --- |
| [Быстрый старт](#быстрый-старт) | Запустить demo и проверить проект локально |
| [Архитектура](#архитектура) | Понять runtime, MCP и границы ответственности |
| [Demo](demo.md) | Проверить проект без личных данных и реальной Jira |
| [Скриншоты](docs/screenshots.md) | Список кадров для публичного GitHub |
| [Roadmap](#roadmap) | Увидеть, что уже сделано и куда проект развивается |
...
| Вопрос | Ответ |
| --- | --- |
| Это RAG? | Нет. В MVP используется metadata index и source pack без vector DB. Это осознанное упрощение ради прозрачности. |
| Почему Jira только read-only? | Потому что для портфолио и QA-workflow важнее безопасная граница, чем эффектная автоматическая запись. |
| Что показывает проект работодателю? | Умение строить AI-assisted процесс вокруг источников, тестов, ограничений и воспроизводимого demo. |
| Можно запустить без Jira и LLM? | Да. Demo vault и fallback позволяют проверить основную идею локально. |
